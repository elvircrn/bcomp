...
void Scanner::Init() {
	...
	noSym = 66;
	int i;
	for (i = 46; i <= 46; ++i) start.set(i, 11);
	for (i = 65; i <= 90; ++i) start.set(i, 11);
	for (i = 95; i <= 95; ++i) start.set(i, 11);
	for (i = 97; i <= 122; ++i) start.set(i, 11);
...
Token* Scanner::NextToken() {
	...
	switch (state) {
		...
		case 3:
			...
			if ((ch >= L'0' && ch <= L'7')) {AddCh(); goto case_3;}
			else {t->kind = 4; break;}
		case 4:
			case_4:
			if ((ch >= L' ' && ch <= L'!') || (ch >= L'#' && ch <= L')') || (ch >= L'+' && ch <= L'~')) {AddCh(); goto case_4;}
			else if (ch == L'"') {AddCh(); goto case_6;}
			else if (ch == L'*') {AddCh(); goto case_5;}
			else {goto case_0;}
		case 5:
			case_5:
			if ((ch >= L' ' && ch <= L'~')) {AddCh(); goto case_4;}
			else {goto case_0;}
		case 6:
			case_6:
			{t->kind = 5; break;}
		case 7:
			if ((ch >= L' ' && ch <= L'&') || (ch >= L'(' && ch <= L')') || (ch >= L'+' && ch <= L'~')) {AddCh(); goto case_8;}
		...
		case 11:
			...
			if (ch == L'.' || (ch >= L'0' && ch <= L'9') || (ch >= L'A' && ch <= L'Z') || ch == L'_' || (ch >= L'a' && ch <= L'z')) {AddCh(); goto case_11;}
			else if (ch == L':') {apx++; AddCh(); goto case_1;}
		...
		case 51:
			...
			else if (ch == L'>') {AddCh(); goto case_58;}
